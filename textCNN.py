...
class Config(object):
    ...
    stopWordSource = './data/stopwords_en.txt'
    numClasses = 4
    rate = 0.8
...
class TextCNN(object):
    ...
    def __init__(self, config, wordEmbedding):
        ...
        self.inputK = tf.placeholder(tf.float32, [None, config.model.kk], name="inputY")
        ...
        with tf.name_scope('cnn'):  # 词向量 卷积核个数，卷积核尺寸，卷积核训练，然后池化最大的,没有激活函数
            convs = []
            for fsz in config.model.filterSizes:
                l_conv = tf.layers.conv1d(self.embeddedWords, 128, fsz, activation=tf.nn.relu)
                l_pool = tf.reduce_max(l_conv, reduction_indices=[1])
                convs.append(l_pool)
        # 得到CNN网络的输出长度
        with tf.name_scope('fc'):  # 输出层，隐藏节点数
            outputs = tf.concat(convs, 1)
            outputs = tf.concat([outputs, self.inputK], 1)
            print(outputs)
            fc = tf.layers.dense(outputs, config.model.hidden_dim, name='fc1')
            fc = tf.nn.dropout(fc, self.dropoutKeepProb)  # 丢弃率
            fc = tf.nn.relu(fc)  # 激活函数

        # 全连接层的输出
        with tf.name_scope("output"):
            self.logits = tf.layers.dense(fc, config.numClasses, name='logits')
...
def nextBatch(x, y, k, batchSize):
    ...
    x_shuffle = x[indices]
    y_shuffle = y[indices]
    k_shuffle = np.array(k)[indices]
    for i in range(num_batch):
        start_id = i * batchSize
        end_id = min((i + 1) * batchSize, data_len)
        yield x_shuffle[start_id:end_id], y_shuffle[start_id:end_id], k_shuffle[start_id:end_id]


def feed_data(x_batch, y_batch, k_batch, keep_prob):
    feed_dict = {
        cnn.inputX: x_batch,
        cnn.inputY: y_batch,
        cnn.inputK: k_batch,
...
def evaluate(sess, x_, y_, k_):
    data_len = len(x_)
    batch_eval = nextBatch(x_, y_, k_, 16)
    total_loss = 0.0
    total_acc = 0.0
    for x_batch, y_batch, k_batch in batch_eval:
        # print(x_batch)
        # print(y_batch)
        batch_len = len(x_batch)
        feed_dict = feed_data(x_batch, y_batch, k_batch, 1.0)
...
def train():
    k_train = KNN_class("./data/ag_train.txt")
    ...
    for epoch in range(config.training.epoches):
        batch_train = nextBatch(trainContents, trainLabels, k_train, config.batchSize)
        print('Epoch:', epoch + 1)
        start = time.time()
        for x_batch, y_batch, k_batch in batch_train:
            feed_dict = feed_data(x_batch, y_batch, k_batch, config.model.dropoutKeepProb)
            ...
            if global_step % config.print_per_batch == 0:
                ...
                val_loss, val_accuracy = evaluate(sess, evalContents, evalLabels, k_train)
...
def test():
    k_test = KNN_class("./data/ag_test.txt")
    sess = tf.Session()
    sess.run(tf.global_variables_initializer())
    saver = tf.train.Saver()
    saver.restore(sess=sess, save_path=save_path)
    print("Testing……")
    test_loss, test_accuracy = evaluate(sess, evalContents, evalLabels, k_test)
    ...
    for i in range(num_batch):
        ...
        feed_dict = {
            ...
            cnn.inputK: k_test[start_id:end_id],
