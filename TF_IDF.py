...
def tf_idf(filePathC):
    vocabList = words()  # 降维后的词典
    # filePathC = "train_set.txt"  # 每个文件名数组
    label, class_df_list, word_list = compute_file(filePathC)
    ...
    for sentence in range(np.array(returnVec).shape[0]):
        vec_List.append(np.array(get_l_tf(np.array(returnVec)[sentence, :])) * idf_array)  # idf*tf
...
def tf_idf_train():
    ...
    # 使用词袋模型统计词语频次
    vocabList = words()  # 降维后的词典
    filePathC = "./data/k_set.txt"  # 从text文件中读取文件
    label, class_df_list, word_list = compute_file(filePathC)
    returnVec, Vec = bagOfWord2Vec(vocabList, word_list)  # 降维后的词典集，returnVec为词典中词语在每个文本中出现的次数，Vec为只要出现过这个词就为1
    df = np.sum(Vec, axis=0)  # 计算tf_idf中的df词语的频率
    idf_array = get_t_idf(df)  # 计算idf
    idf_array = np.array(idf_array)
    train_vec_List = []
...
if __name__ == '__main__':
    print(tf_idf("./data/k_set.txt"))  # 经过tf_idf后的向量
