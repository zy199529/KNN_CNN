...
def vocablist():
    filepath = "./data/k_set.txt"
    label, class_df_list, word_list = compute_file(filepath)
    term_set = createVocabList(word_list)
    print(len(term_set))
    term_class_df_mat = []
...
def feature_selection_ig():  # 计算每个词的信息熵，按信息熵的从大到小返回词语
    ...
    for index in sorted_term_score_index:
        IG_word.append((str(term_set[index]), term_score_array[index]))
    return IG_word
